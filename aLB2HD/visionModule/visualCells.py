...
class visualInputs():
    ...
    def __init__(self, N_bin, N_cue, Angle, precision_visual1, precision_visual2, proximal_length, Fv_max_factor):
        ...
        self.F_visual_temp = self.F_visual_temp / \
            np.max(self.F_visual_temp) * Fv_max_factor
        ...
        F_visual_feature_max = np.max(
            self.F_visual_feature, axis=1, keepdims=True)
        self.F_visual_feature = (
            Fv_max_factor / F_visual_feature_max) * self.F_visual_feature
        self.F_visual_feature_mean = np.mean(
            self.F_visual_feature, axis=1, keepdims=True)
        self.F_visual_feature = (
            self.F_visual_mean / self.F_visual_feature_mean) * self.F_visual_feature

        # Compute the autocorrelation
        F_visual_feature_autocorr = np.sum(
            self.F_visual_feature.T**2, axis=0) / self.F_visual_feature.shape[1]
        # Compute the norm
    ...
    def _add_red_cue(self, angle, precision_visual2):
        self.F_visual_feature[0, :] = vonmisespdf(
            angle, 90, precision_visual2) + vonmisespdf(angle, -90, precision_visual2)

    def _add_blue_cue(self, angle, precision_visual2, proximal_length):
        p_values = np.arange(-proximal_length / 2, proximal_length / 2 + 1)
        for p in p_values:
            self.F_visual_feature[1,
                                  :] += vonmisespdf(angle, p, precision_visual2)
...
class visualCells(DefaultParameters):
    ...
    def _initialize_visual_field(self):
        ...
        '''
        self.visual_field = vonmisespdf(
            self.params.Angle, 0, self.params.precision_visualfield)
    ...
    def _initialize_feature_attention(self):
        ...
        '''
        self.feature_attention = np.ones(
            self.params.N_cue) / self.params.N_cue * self.params.total_featural_attention
    ...
    def _initialize_neural_attention(self):
        ...
        self.neural_attention = np.repeat(
            self.feature_attention, self.params.N_bin)
        self.neural_attention = self.neural_attention.reshape(
            self.params.N_cue, self.params.N_bin)
        self.neural_attention = self.neural_attention.flatten()
    ...
    def get_visual_shift(self, present_env, trajectory, curr_time):
        self.cue = vAngularDiff(
            self.params.Cue_global[present_env, :], trajectory[curr_time])
        self.strength = self.params.Strength_global[present_env, :]
        shift = np.round(self.cue/self.params.angle_gap, 0).astype(int)
        F_visual_shift = np.roll(self.Inputs.F_visual_feature, shift, 1)

        return F_visual_shift

    def update_F_visual(self, F_visual_shift, test=False):
        '''
        ...
        self.F_visual_shiftc = F_visual_shift.flatten()
        self.F_visual_shiftc = self.F_visual_shiftc.reshape(
            1, self.F_visual_shiftc.shape[0])
        self._add_visual_noise(test)
        F_visual_shift[F_visual_shift < 0] = 0
        self.F_visual = self.F_visual_shiftc * strength_expand

    def _add_visual_noise(self, test=False):
        '''
        add visual noise to the visual field
        '''
        if not test:
            noise = 2*np.random.rand(self.params.N_cue * self.params.N_bin) - 1
            noise = noise / \
                np.sum(np.abs(noise)) * \
                self.params.visual_noise_intensity * self.params.N_bin
            self.F_visual_shiftc += noise
        else:
            pass
