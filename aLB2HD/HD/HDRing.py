...
class HDattractor:
    ...
    def _get_recurrent_column_weights(self):
        ...
        F0 = vonmisespdf(
            self.params.Angle, self.params.weight_bias, self.params.precision_HD
        )
        F0 = (F0 / F0.max()) * 0.8

        # get activation level from the firing rate
        U0 = inverseFR_sigmoid(
            F0, self.params.alpha_HD, self.params.beta_HD, self.params.gamma_HD
        )
        self.U0 = U0
    ...
    def _dw_HD(self):
        self.dw_t = Differentiation_ascending(
            self.recurrent_column_weight, self.params.angle_gap
        ) + self.params.weight_noise_scale * np.random.randn(self.params.N_bin)
    ...
    def get_angular_acceleration(self, trajectory):
        return Differentiation_ascending(
            self.get_aneular_velocity(trajectory), self.params.dt
        )
    ...
    def _compute_gamma(self, velocity, acceleration):
        ...
        gamma = -self.params.time_constant_HD * (
            velocity + self.params.rho * acceleration
        )
        gamma = gamma + self.params.asyweightstrength_noise_scale * np.random.randn(
            self.T_len
        )  # add noise
    ...
    def _hd_trajectory_Niflheim(self):
        ...
        self.real_trajectory = self.real_hdTrajectory[
            int(initiation) : int(self.params.T_len + initiation) + 1
        ]
        trajectory_0 = self.real_hdTrajectory[0]
        self.real_trajectory[0] = 0

        for i in range(1, self.params.T_len):
            self.real_trajectory[i] = AngularDiff(self.real_trajectory[i], trajectory_0)

            # check if the angle between consecutive points is larger than 30

            if (
                np.abs(
                    AngularDiff(self.real_trajectory[i], self.real_trajectory[i - 1])
                )
                > 30
            ):
    ...
    def _hd_trajectory_Noatun(self):
        """poisoning scenario"""
        ppt_hd = (
            np.random.rand(1, self.params.N_env) * 360 - 180
        ...
        for i in range(0, self.params.T_len - 1):
            ...
            if (
                np.where(
                    self.params.time_CueShifting
                    >= (self.params.beginning + (i + 1) * self.params.dt_exp)
                )[0]
                != np.where(
                    self.params.time_CueShifting
                    >= (self.params.beginning + i * self.params.dt_exp)
                )[0]
            ):
                poisoned_hd = ppt_hd[
                    np.where(
