...
class vis2aLBPathway():
    ...
    def __init__(self,
        ...
        if wandering == 1 and wanderingTime is None:
            raise ValueError(
                "wanderingTime must be specified when the agent is allowed to wander")
        ...
        self.params.Inhibition_U_arep = (np.ones(self.params.N_abstract) - np.eye(
            self.params.N_abstract))/np.sqrt(self.params.N_abstract - 1)

        # time handling

        self.training_idicies = np.arange(
            0 + self.params.dt_exp, self.params.time+self.params.dt_exp, self.params.dt_exp)
        self.params.time_CueShifting = np.arange(
            0 + (self.params.time / N_env), self.params.time + (self.params.time / N_env), self.params.time / N_env)
        self.stop_learning_time = self.params.time
        self.params.T_len = len(self.training_idicies)
        ...
        self.HDModule = HDattractor(self.params)
    ...
    def train(self, store=False, interval=50, mode='all', last_nb: int = 10000):
        ...
        if store:
            if mode == 'all':
                ...
                self.vis2ALBweights = np.zeros(
                    (recordTimePoints, self.params.N_abstract, self.params.N_input))
                # firing rates
                self.aLB_fr = np.zeros(
                    (recordTimePoints, self.params.N_abstract))

            elif mode == 'last':
                start_idx = self.T_len - last_nb
                self.vis2ALBweights = np.zeros(
                    (last_nb, self.params.N_abstract, self.params.N_input))
        ...
        for i in tqdm(range(self.T_len), desc='Training'):
            ...
            if self.params.wandering:
                present_env = np.round(
                    curr_time_global / self.params.wanderingTime, 0) % self.params.N_env + 1

            else:
                if previous_env < self.params.N_env:
                    present_env = np.argmax(self.params.time_CueShifting >= curr_time_global) if np.any(
                        self.params.time_CueShifting >= curr_time_global) else None
            ...
            shift = self.VisualModule.get_visual_shift(
                present_env-1, self.HDModule.trajectory, i)
            self.VisualModule.update_F_visual(shift)
            ...
            # lr update
            self.ALBModule.update_lr(
                i, curr_time_global, self.stop_learning_time)
            ...
            if store:
                if mode == 'all':
                    if store and i % interval == 0:
                        self.vis2ALBweights[i //
                                            interval] = self.ALBModule.W_vis2aLB.copy()
                        self.aLB_fr[i //
                                    interval] = self.ALBModule.F_arep.copy()
    ...
    def test(self, store=False):
        ...
        start_time = 0
        end_time = 60
        test_idicies = np.arange(
            start_time, end_time+self.params.dt, self.params.dt)
        cycle = 10
        ...
        # data collection
        self.recorded_aLB_fr = np.zeros(
            (len(bar_angle), self.params.N_abstract))

        # test HD trajectory
        self.test_HD_trajectory = np.zeros(self.testing_T_len)
        self.test_HD_trajectory[0] = self.HDModule.trajectory[-1]
        ...
        for i in tqdm(range(self.testing_T_len-1), desc='Testing'):
            ...
            # HD handling
            self.test_HD_trajectory[i+1] = AngularDiff(
                self.test_HD_trajectory[i] + vlcty * self.params.dt, 0)
            ...
            shift = self.VisualModule.get_visual_shift(
                present_env-1, self.HDModule.trajectory, i)
            self.VisualModule.update_F_visual(shift, test=True)
            ...
            if store:
                ...
                mask = (present_HD_angle >= bar_angle) & (
                    present_HD_angle < (bar_angle + bar_angle_gap))
                indcies = np.where(mask)[0]
                if indcies.size > 0:  # only record the first matched angle
                    idx = indcies[0]
                    self.recorded_aLB_fr[idx,
                                         :] += self.ALBModule.F_arep.copy()

        self.norm_recorded_aLB_fr = self.recorded_aLB_fr.T * \
            self.params.dt/((end_time - start_time) / (360/bar_angle_gap))
        print('Testing completed')

        self.plot(bar_angle_gap, end_time, start_time,
                  len(bar_angle), present_env)
    ...
    def plot(self, bar_angle_gap, end_testing_time, start_testing_time, bar_bin, test_env):
        ...
        self.norm_recorded_aLB_fr = self.recorded_aLB_fr.T * self.params.dt / \
            ((end_testing_time - start_testing_time) / (360 / bar_angle_gap))
        # self.norm_recorded_aLB_fr = self.norm_recorded_aLB_fr / \
        #     np.max(self.norm_recorded_aLB_fr, axis=1, keepdims=True)

        # self.norm_recorded_aLB_fr = self.recorded_aLB_fr.T / np.max(self.recorded_aLB_fr.T, axis=1, keepdims=True)
        
        Bar_total_activated_unit = 0
        firingrate_criterion_test = self.params.firingrate_cirterion

        while Bar_total_activated_unit < 2:
            Bar_central_angle_index = np.zeros(
                self.params.N_abstract, dtype=int)
            for l in range(self.params.N_abstract):
                if np.max(self.norm_recorded_aLB_fr[l, :]) > firingrate_criterion_test:
                    Bar_central_angle_index[l] = np.argmax(
                        self.norm_recorded_aLB_fr[l, :])
                else:
                    Bar_central_angle_index[l] = bar_bin + 1

            bar_RemapIndex = np.argsort(Bar_central_angle_index)
            Bar_total_activated_unit = np.sum(
                Bar_central_angle_index <= bar_bin)
        ...
        if Bar_total_activated_unit > 0:
            Bar_MapSorted_AstRepresentation = np.zeros(
                (Bar_total_activated_unit, bar_bin))
            Bar_FWHM = np.zeros(Bar_total_activated_unit)
            for l in range(Bar_total_activated_unit):
                Bar_MapSorted_AstRepresentation[l,
                                                :] = self.norm_recorded_aLB_fr[bar_RemapIndex[l], :]
                Bar_FWHM[l] = np.sum(Bar_MapSorted_AstRepresentation[l, :] >= np.max(
                    Bar_MapSorted_AstRepresentation[l, :]) / 2) * bar_angle_gap
        ...
        # First subplot
        X, Y = np.meshgrid(np.arange(-180, 180, bar_angle_gap),
                           np.arange(1, self.params.N_abstract + 1))
        c1 = ax[0].contourf(
            X, Y, self.norm_recorded_aLB_fr[:360, :360], cmap='viridis')
        ...
        # Second subplot
        X, Y = np.meshgrid(np.arange(-180, 180, bar_angle_gap),
                           np.arange(1, Bar_total_activated_unit + 1))
        c2 = ax[1].contourf(
            X, Y, Bar_MapSorted_AstRepresentation[:360, :360], cmap='viridis')
    ...
    def save_training_data(self, filepath, store=True):
        ...
        np.save(rp+'/F_featural_attention.npy',
                self.VisualModule.Inputs.F_visual_feature)
...
def main():
    ...
    vis2aLB = vis2aLBPathway(params, duration, N_env, N_cue, Cue_Init,
                             Strength_Init, firingrate_cirterion, HDdata, wandering, wanderingTime)
