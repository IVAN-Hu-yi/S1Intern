...
class ALBcells():
    ...
    def _initialised_vis2aLB_weight(self):
        ...
        self.W_vis2aLB = np.random.rand(
            self.params.N_abstract, self.params.N_input)
    ...
    def update_U_arep(self, VisualModule: visualCells, curr_time: int, weights=None):
        ...
        # Update the abstract representation units
        visualComponent = self.params.Uv_gain_factor * \
            VisualModule.neural_attention * VisualModule.F_visual_p @ weights.T
        modulatedVisualComponent = self.theta_ff[curr_time] * visualComponent
        visualInput = np.asarray(modulatedVisualComponent).flatten()

        lateralInhibition = self.params.inhibition_U_arep * \
            self.F_arep_p @ self.Inhibition_U_arep.T
    ...
    def update_F_arep(self, test=False):
        ...
        if test:
            self.F_arep = firing_rate_sigmoid(
                self.U_arep_p, self.params.alpha_arep, self.params.beta_arep, self.params.gamma_arep)
        else:
            self.F_arep = firing_rate_sigmoid(
                self.U_arep, self.params.alpha_arep, self.params.beta_arep, self.params.gamma_arep)
    ...
    def vis2aLB_weight_update(self, VisualModule: visualCells, curr_time: int):
        ...
        self.W_vis2aLB = mOSA(VisualModule.F_visual_p, self.F_arep, self.W_vis2aLB, self.lr, self.theta_ff[curr_time], self.theta_fb[curr_time])

    def update_lr(self, curr_time, present_time_global, stop_learning_time):
        '''
        update the learning rate
        '''
        self.lr = (present_time_global <= stop_learning_time) * self.params.lr_initial_rate_visual * \
            np.exp(-self.params.lr_decay_rate_visual *
                   curr_time * self.params.dt)
